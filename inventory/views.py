...
from .forms import (
    ...
    ClientForm,
    ClientOrderForm,
    ClientOrderLineFormSet,
    ProductionRunForm,
    ProductionRunAllocationForm,
    ProductionComponentForm,
    ProductionComponentFormSet,
)
...
class DashboardView(View):
    def get(self, request):
        ...
        supply_order_counts = dict(
            Counter(SupplyOrder.objects.values_list("status", flat=True))
        )
        client_order_counts = dict(
            Counter(
...
class ClientOrderBoardView(View):
    def get(self, request):
        ...
        delivered = (
            ...
            .select_related("client")
            .exclude(date_delivered__lt=cutoff)
            .order_by("-date_delivered")
        )
        return render(
            request,
            "client_orders/order_board.html",
            {
                "placed": placed,
...
class SalesOrderListView(View):
    def get(self, request):
        ...
        orders = SalesOrder.objects.select_related("client", "carrier").order_by(
            "-order_date"
        )
        if q:
...
class SalesOrderDetailView(View):
    def get(self, request, pk):
        order = get_object_or_404(
            SalesOrder.objects.select_related("client", "carrier"), pk=pk
        )
        from .models import (
            ProductBatch,
            ProductBatchReservation,
    ...
    def post(self, request, pk):
        ...
        if action == "update_status":
            ...
            if new_status in dict(SalesOrder.STATUS_CHOICES):
                ...
                messages.success(
                    request,
                    f"{order.reference} updated to {order.get_status_display()}.",
                )
                return redirect("client-order-board")
    ...
    def get(self, request, pk):
        ...
        return render(
            ...
            "carriers/form.html",
            {
                ...
                "form_title": f"Edit {carrier.name}",
                "submit_label": "Save",
                "carrier": carrier,
            },
        )
...
class SupplierDetailView(View):
    def get(self, request, pk):
        ...
        supply_orders = SupplyOrder.objects.filter(supplier=supplier).order_by(
            "-order_date"
        )[:10]
        contacts = [
    ...
    def get(self, request, pk):
        ...
        return render(
            ...
            "suppliers/form.html",
            {
                ...
                "form_title": f"Edit {supplier.name}",
                "submit_label": "Save",
                "supplier": supplier,
            },
        )
...
class SupplyOrderBoardView(View):
    def get(self, request):
        ...
        cutoff = date.today() - timedelta(days=5)
        placed = (
            SupplyOrder.objects.filter(status="ORDER_PLACED")
            .select_related("supplier", "carrier")
            .order_by("expected_delivery")
        )
        dispatched = (
            SupplyOrder.objects.filter(status="DISPATCHED")
            .select_related("supplier", "carrier")
            .order_by("expected_delivery")
        )
        delivered = (
            SupplyOrder.objects.filter(status="DELIVERED")
            .select_related("supplier", "carrier")
            .exclude(date_delivered__lt=cutoff)
            .order_by("-date_delivered")
        )
...
class SupplyOrderListView(View):
    def get(self, request):
        q = request.GET.get("q", "").strip()
        status = request.GET.get("status", "").strip()
        orders = SupplyOrder.objects.select_related("supplier", "carrier").order_by(
            "-order_date"
        )
        if q:
            orders = orders.filter(
...
class UnitListView(View):
    def get(self, request):
        return render(
            request, "units/list.html", {"units": Unit.objects.all().order_by("name")}
        )
    ...
    def get(self, request, pk):
        ...
        return render(
            ...
            "units/form.html",
            {
                ...
                "form_title": f"Edit Unit: {unit.name}",
                "submit_label": "Save Changes",
            },
        )
...
class LocationDetailView(View):
    ...
    def get(self, request, pk):
        ...
        raw_batches = (
            RawMaterialBatch.objects.filter(location=location)
            .select_related("material__unit")
            .order_by("-created_at")
        )
        product_batches = (
            ProductBatch.objects.filter(location=location)
            .select_related("material__unit")
            .order_by("-created_at")
        )
        return render(
...
class MaterialDetailView(View):
    def get(self, request, pk):
        ...
        if m.category == "FIN":
            runs = (
                ProductionRun.objects.filter(material=m)
                .prefetch_related(
                    Prefetch(
                        "components",
                        queryset=ProductionComponent.objects.select_related("material"),
                    )
                )
        ...
        material = m
        raw_batches = (
            list(RawMaterialBatch.objects.filter(material=m).select_related("location"))
            if m.category in ("RAW", "PKG")
            else []
        )
        prod_batches = (
            list(ProductBatch.objects.filter(material=m).select_related("location"))
            if m.category == "FIN"
            else []
        )
        ...
        if m.category in ("RAW", "PKG"):
            ...
            allocated = RBA.objects.filter(raw_batch__material=m).aggregate(
                t=_Sum("quantity")
            )["t"] or Decimal("0")
            ...
            reserved = PBR.objects.filter(product_batch__material=m).aggregate(
                t=_Sum("quantity_reserved")
            )["t"] or Decimal("0")
...
class RawMaterialBatchListView(View):
    def get(self, request):
        ...
        qs = qs.annotate(
            ...
            computed_available=F("total_quantity")
            - Coalesce(
                Sum("allocations__quantity"), Decimal("0"), output_field=DecimalField()
            )
        )
    ...
    def get(self, request, pk):
        ...
        eligible_runs_qs = (
            ProductionRun.objects.filter(
                components__material=batch.material, status__in=["PLANNED", "ACTIVE"]
            )
        ...
        for run in eligible_runs_qs:
            ...
            already_allocated = RawBatchAllocation.objects.filter(
                production_run=run, raw_batch__material=batch.material
            ).aggregate(
                total=Coalesce(
        ...
        allocations = RawBatchAllocation.objects.filter(raw_batch=batch).select_related(
    ...
    def post(self, request, pk):
        ...
        if action == "update_status":
            ...
            if new_status in dict(RawMaterialBatch.STATUS_CHOICES):
                ...
                batch.save()
                messages.success(
                    request, f"Status updated to {batch.get_status_display()}."
                )
            ...
            try:
                ...
                if qty > available:
                    ...
                    if qty > still_needed:
                        ...
                        messages.success(
                            request, f"Allocated {qty} units to {run.reference}."
                        )
            ...
            try:
                ...
                total_allocated = (
                    RawBatchAllocation.objects.filter(raw_batch=batch)
                    .exclude(pk=alloc.pk)
                    .aggregate(
                        total=Coalesce(
                            Sum("quantity"), Decimal("0"), output_field=DecimalField()
...
class RawMaterialBatchCreateView(View):
    def get(self, request):
        ...
        raw_mats = list(
            Material.objects.filter(category__in=["RAW", "PKG", "FIN", "CON"])
            .order_by("name")
            .values("id", "name", "sku")
        )
        return render(
...
class ProductBatchListView(View):
    def get(self, request):
        ...
        return render(
            request,
            "product_batches/list.html",
            {
                "batches": paginator.get_page(request.GET.get("page")),
                "all_materials": Material.objects.filter(category="FIN").order_by(
                    "name"
                ),
                "all_locations": Location.objects.order_by("name"),
                "current_sort": sort,
...
class ProductBatchDetailView(View):
    def get(self, request, pk):
        ...
        transactions = (
            MaterialTransaction.objects.filter(product_batch=batch)
            .select_related("raw_material_batch__material")
            .order_by("-created_at")
        )
        summary = {}
        ...
        reservations = ProductBatchReservation.objects.filter(
            product_batch=batch, order_line__isnull=False
        ...
        eligible_lines = (
            ClientOrderLine.objects.filter(material=batch.material)
    ...
    def post(self, request, pk):
        ...
        if action == "reserve":
            ...
            try:
                ...
                if qty > available:
                    ...
                    line.quantity_fulfilled = (
                        line.quantity_fulfilled or Decimal("0")
                    ) + qty
                    ...
                    line.save()
                    messages.success(
                        request, f"Reserved {qty} units for {line.order.reference}."
                    )
            ...
            try:
                res = ProductBatchReservation.objects.get(
                    pk=res_id, product_batch=batch
                )
                line = res.order_line
                qty = res.quantity_reserved
                res.delete()
                # Reverse the fulfilled quantity
                line.quantity_fulfilled = max(
                    Decimal("0"), (line.quantity_fulfilled or Decimal("0")) - qty
                )
...
class ProductBatchCreateView(View):
    def get(self, request):
        ...
        if request.GET.get("material"):
            form.initial["material"] = request.GET["material"]
        fin_mats = list(
            Material.objects.filter(category="FIN")
            .order_by("name")
            .values("id", "name", "sku")
        )
        return render(
            request,
            "product_batches/form.html",
            {
                "form": form,
...
class ProductBatchEditView(View):
    def get(self, request, pk):
        batch = get_object_or_404(
            ProductBatch.objects.select_related(
                "material__unit", "location", "production_run__material__unit"
            ),
            pk=pk,
        )
        fin_mats = list(
    ...
    def get(self, request):
        ...
        return render(
            ...
            "transactions/list.html",
            {
                ...
                "all_batches": RawMaterialBatch.objects.select_related(
                    "material"
                ).order_by("-created_at"),
                "all_product_batches": ProductBatch.objects.select_related(
                    "material"
                ).order_by("-created_at"),
            },
        )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, f"Reserved {form.cleaned_data['quantity']} units."
                )
        ...
        return (
            redirect("batch-detail", pk=batch_id)
            if batch_id
            else redirect("batch-list")
        )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, f"Consumed {form.cleaned_data['quantity']} units."
                )
            except ValidationError as e:
                messages.error(request, e.message)
        else:
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, f"Released {form.cleaned_data['quantity']} units."
                )
            except ValidationError as e:
                messages.error(request, e.message)
        else:
...
class ClientListView(View):
    def get(self, request):
        ...
        return render(
            request,
            "client_orders/client_list.html",
...
class ClientDetailView(View):
    def get(self, request, pk):
        ...
        orders = (
            ClientOrder.objects.filter(client=client)
            .prefetch_related("lines")
            .order_by("-order_date")
        )
        return render(
            request,
            "client_orders/client_detail.html",
            {
                "client": client,
                "orders": orders,
            },
...
class ClientDeleteView(View):
    def post(self, request, pk):
        ...
        try:
            ...
            messages.error(
                request, f'Cannot delete "{name}" — they have existing orders.'
            )
        return redirect("client-list")
...
class ShippedOrdersListView(View):
    def get(self, request):
        orders = (
            ClientOrder.objects.filter(status="SHIPPED")
            .select_related("client")
            .order_by("-date_shipped", "-created_at")
        )
        return render(
            request,
            "client_orders/shipped_list.html",
            {
                "orders": orders,
            },
        )
    ...
    def get(self, request, pk):
        order = get_object_or_404(
            ClientOrder.objects.select_related("client").prefetch_related(
                Prefetch(
                    "lines",
                    queryset=ClientOrderLine.objects.select_related(
                        "material__unit"
                    ).prefetch_related(
                        Prefetch(
                            "batch_reservations",
                            queryset=ProductBatchReservation.objects.select_related(
                                "product_batch"
                            ),
                        )
                    ),
        ...
        pb_reserved = {
            ...
            for r in ProductBatchReservation.objects.filter(order_line__isnull=False)
            .values("product_batch_id")
            .annotate(total=_Sum("quantity_reserved"))
        }
        pb_available = {}
        ...
        pb_available_json = _json.dumps(
            {str(k): float(v) for k, v in pb_available.items()}
        )
        ...
        reserved_batch_ids = (
            ProductBatchReservation.objects.filter(order_line__order=order)
            .values_list("product_batch_id", flat=True)
            .distinct()
        )

        # Step 2: find production runs linked to those batches
        run_ids_from_batches = (
            ProductionRun.objects.filter(product_batch_id__in=reserved_batch_ids)
            .values_list("id", flat=True)
            .distinct()
        )

        # Also include runs pre-reserved directly against this order's lines
        run_ids_from_reservations = (
            ProductionRunReservation.objects.filter(order_line__order=order)
            .values_list("production_run_id", flat=True)
            .distinct()
        )
        ...
        run_ids = list(
            set(list(run_ids_from_batches) + list(run_ids_from_reservations))
        )
        ...
        for comp in components:
            ...
            boq[m.pk]["material"] = m
            boq[m.pk]["required"] += (comp.quantity_required * proportion).quantize(
                Decimal("0.001")
            )
        ...
        for entry in boq.values():
            ...
            if m.category == "FIN":
                ...
                for pb in fin_batches:
                    reserved = (
                        ProductBatchReservation.objects.filter(product_batch=pb)
                        .exclude(order_line__order=order)
                        .aggregate(
                            total=Coalesce(
                                Sum("quantity_reserved"),
                                Decimal("0"),
                    ...
                    total_available += max(
                        Decimal("0"), pb.quantity_produced - reserved
                    )
            else:
                ...
                for b in batches:
                    ...
                    other_allocated = (
                        RawBatchAllocation.objects.filter(raw_batch=b)
                        .exclude(production_run_id__in=run_ids)
                        .aggregate(
                            total=Coalesce(
                                Sum("quantity"),
                                Decimal("0"),
                    ...
                    total_available += max(
                        Decimal("0"), b.total_quantity - other_allocated
                    )
            ...
            worst = (
                min(
                    entry["statuses"],
                    key=lambda s: (
                        STATUS_PRIORITY.index(s) if s in STATUS_PRIORITY else 0
                    ),
                )
                if entry["statuses"]
                else "PENDING"
        ...
        boq_rows.sort(
            key=lambda r: (
                STATUS_PRIORITY.index(r["status"])
                if r["status"] in STATUS_PRIORITY
                else 0,
                r["material"].name,
            )
        )
        ...
        if not lines:
            ...
            fulfilment_status = min(
                line_statuses, key=lambda s: priority.index(s) if s in priority else 0
            )
        ...
        eligible_runs = (
            ProductionRun.objects.filter(material_id__in=line_materials)
            .exclude(status="CANCELLED")
            .select_related("material")
            .order_by("-created_at")
        )
        ...
        return render(
            request,
            "client_orders/order_detail.html",
            {
                "order": order,
                "boq_rows": boq_rows,
    ...
    def post(self, request, pk):
        ...
        if action == "ship":
            ...
            for res in reservations:
                ...
                pb.save()
                deducted[pb.batch_number] = (
                    deducted.get(pb.batch_number, Decimal("0")) + qty
                )
            ...
            if new_status in dict(ClientOrder.STATUS_CHOICES):
                ...
                order.save()
                messages.success(
                    request, f"Order status updated to {order.get_status_display()}."
                )
            ...
            try:
                ...
                if run.material != line.material:
                    messages.error(
                        request, "Run product does not match order line material."
                    )
                else:
                    ...
                    line_remaining = (
                        line.quantity_ordered - line_reserved_batch - line_reserved_run
                    )
                    if qty > line_remaining:
                        messages.error(
                            request,
                            f"Only {line_remaining} units still needed for this line.",
                        )
                    else:
                        ...
                        messages.success(
                            request, f"Reserved {qty} units from run {run.reference}."
                        )
            except (
                ClientOrderLine.DoesNotExist,
                ProductionRun.DoesNotExist,
                ValueError,
                InvalidOperation,
            ) as e:
            ...
            try:
                ...
                if batch.material != line.material:
                    messages.error(
                        request, "Batch material does not match order line material."
                    )
                else:
                    ...
                    if qty > batch_available:
                        messages.error(
                            request,
                            f"Only {batch_available} units available in this batch.",
                        )
                    elif qty > line_remaining:
                        messages.error(
                            request,
                            f"Only {line_remaining} units still needed for this order line.",
                        )
                    else:
                        ProductBatchReservation.objects.create(
                            product_batch=batch,
                        ...
                        line.save()
                        messages.success(
                            request, f"Reserved {qty} units from {batch.batch_number}."
                        )
            except (
                ClientOrderLine.DoesNotExist,
                ProductBatch.DoesNotExist,
                ValueError,
                InvalidOperation,
            ) as e:
                messages.error(request, f"Error: {e}")
...
class ClientOrderCreateView(View):
    def get(self, request):
        ...
        clients_list = list(
            Client.objects.order_by("name").values("id", "name", "code")
        )
        return render(
            request,
            "client_orders/order_form.html",
            {
    ...
    def post(self, request, pk):
        ...
        if deleted_ids:
            ClientOrderLine.objects.filter(
                pk__in=[int(x) for x in deleted_ids if x.isdigit()], order=order
            ).delete()
    ...
    def get(self, request, pk):
        run = get_object_or_404(
            ProductionRun.objects.select_related(
                ...
                Prefetch(
                    "components",
                    queryset=ProductionComponent.objects.select_related(
                        "material__unit"
                    ).order_by("material__name"),
                ),
                Prefetch(
                    "allocations",
                    queryset=ProductionRunAllocation.objects.select_related(
        ...
        linkable_batches = (
            ProductBatch.objects.filter(material=run.material)
            .select_related("material")
            .order_by("-created_at")
        )
        ...
        for comp in run.components.select_related("material").all():
            batches = (
                RawMaterialBatch.objects.filter(material=comp.material)
            ...
            if comp.material.category == "FIN":
                fin_batches = (
                    ProductBatch.objects.filter(material=comp.material)
                    .select_related("location")
                    .order_by("location__name")
                )
                for pb in fin_batches:
    ...
    def post(self, request, pk):
        ...
        if action == "update_status":
            ...
            if new_status == "COMPLETED":
                ...
                if not run.all_components_in_warehouse:
                    ...
                    if not PB.objects.filter(batch_number=batch_num).exists():
                        pb = PB.objects.create(
                            ...
                            quantity_produced=run.actual_quantity
                            or run.planned_quantity,
                            location=run.location,
                        )
                        run.product_batch = pb
                        run.save()
                    # Auto-transfer any ProductionRunReservations to the new ProductBatch
                    for prr in ProductionRunReservation.objects.filter(
                        production_run=run
                    ):
                ...
                run.save()
                messages.success(
                    request, f"Run status updated to {run.get_status_display()}."
                )
            ...
            if form.is_valid():
                ...
                if existing + qty > run.planned_quantity:
                    ...
                    messages.error(
                        request, "This order line is already allocated to this run."
                    )
                else:
            ...
            try:
                ...
                if is_fin:
                    ...
                    if not comp:
                        messages.error(
                            request,
                            f"{pb.material.name} is not a component of this run.",
                        )
                    else:
                        reserved = ProductBatchReservation.objects.filter(
                            product_batch=pb
                        ...
                        if qty > available:
                            messages.error(
                                request,
                                f"Only {available} units available in that batch.",
                            )
                        else:
                            still_needed = max(
                                Decimal("0"), comp.quantity_required - reserved
                            )
                            if qty > still_needed:
                                messages.error(
                                    request,
                                ...
                                messages.success(
                                    request,
                                    f"Allocated {qty} units from {pb.batch_number}.",
                                )
                else:
                    ...
                    if not comp:
                        messages.error(
                            request,
                            f"{batch.material.name} is not a component of this run.",
                        )
                    else:
                        total_allocated = RawBatchAllocation.objects.filter(
                            raw_batch=batch
                        ...
                        if qty > available:
                            ...
                            still_needed = max(
                                Decimal("0"), comp.quantity_required - already_allocated
                            )
                            if qty > still_needed:
                                messages.error(
                                    request,
                                ...
                                messages.success(
                                    request,
                                    f"Allocated {qty} units from {batch.lot_number}.",
                                )
            except (
                RawMaterialBatch.DoesNotExist,
                ProductBatch.DoesNotExist,
                ValueError,
                InvalidOperation,
            ) as e:
            ...
            if batch_id:
                try:
                    ...
                    if pb.material != run.material:
                        ...
                        has_existing_qty = (
                            pb.quantity_produced and pb.quantity_produced > 0
                        )
            ...
            try:
                ...
                if qty_dec <= 0:
                    raise ValueError
                order_line = (
                    ClientOrderLine.objects.get(pk=int(order_line_id))
                    if order_line_id
                    else None
                )
                ...
                run.save()
                messages.success(
                    request, f"Run {run.reference} shipped and moved to history."
                )
...
class ProductionTemplateLookupView(View):
    ...
    def get(self, request):
        ...
        try:
            template = ProductionTemplate.objects.select_related("product").get(
...
class ProductionRunCreateView(View):
    def get(self, request):
        ...
        return render(
            request,
            "production_runs/form.html",
            {
                "form": form,
                "formset": formset,
                "form_title": "New Production Run",
                "submit_label": "Create Run",
    ...
    def get(self, request, pk):
        ...
        return render(
            ...
            "production_runs/form.html",
            {
                ...
                "form_title": f"Edit Run: {run.reference}",
                "submit_label": "Save Changes",
                "run": run,
                "all_raw_materials_json": json.dumps(raw_mats),
                "fin_materials_json": json.dumps(fin_mats),
...
class ProductionComponentUpdateView(View):
    ...
    def post(self, request, pk):
        ...
        if new_status in dict(ProductionComponent.STATUS_CHOICES):
            ...
            if (
                new_status in ("IN_WAREHOUSE_RAW", "IN_PROCESS", "FINAL_PRODUCT")
                and not component.actual_date
            ):
                component.actual_date = timezone.now().date()
            component.save()
            messages.success(
                request, f"{component.material.name} → {component.get_status_display()}"
            )
    ...
    def get(self, request):
        ...
        return render(
            ...
            "allocations/list.html",
            {
                ...
                "all_runs": ProductionRun.objects.select_related("material").order_by(
                    "-created_at"
                ),
                "all_orders": ClientOrder.objects.select_related("client").order_by(
                    "-order_date"
                ),
            },
        )
    ...
    def get(self, request):
        ...
        if order_id:
            try:
                ...
                material_ids = (
                    ProductBatchReservation.objects.filter(
                        order_line__order=selected_order
                    )
                    .values_list("product_batch__material_id", flat=True)
                    .distinct()
                )
                run_ids_from_reservations = (
                    ProductionRunReservation.objects.filter(
                        order_line__order=selected_order
                    )
                    .values_list("production_run_id", flat=True)
                    .distinct()
                ...
                runs = (
                    ProductionRun.objects.exclude(status__in=["COMPLETED", "CANCELLED"])
                    ...
                    .filter(
                        ...
                        | Q(pk__in=run_ids_from_reservations)
                    )
        ...
        orders = (
            ClientOrder.objects.exclude(status__in=["SHIPPED", "CANCELLED"])
            .select_related("client")
            .order_by("-order_date")
        )
...
class ShipmentHistoryView(View):
    ...
    def get(self, request):
        ...
        return render(
            request,
            "production_runs/shipment_history.html",
            {
    ...
    def get(self, request):
        ...
        for prefix, lt in DEFAULT_LEAD_TIMES.items():
            LeadTimeConfig.objects.get_or_create(
                sku_prefix=prefix, defaults={"lead_time_months": lt}
            )
        ...
        if sort in (
            ...
            rows = sorted(
                rows,
                key=lambda r: (
                    (r[sort] or 0) if sort != "sku" and sort != "name" else r[sort]
                ),
                reverse=reverse,
            )
        ...
        if action == "save_settings":
            ...
            if form.is_valid():
                ...
                chosen = [
                    ...
                    if request.POST.get(
                        f"month_{lbl.replace('/', '').replace('-', '')}"
                    )
                ]
                obj.selected_months = (
                    chosen  # save exactly what was checked (may be empty)
                )
...
class ReorderAlertsExportView(View):
    def get(self, request):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="reorder-alerts-{date.today()}.xlsx"'
        )
        wb.save(response)
        return response
...
class ReorderComponentsView(View):
    def get(self, request):
        ...
        if not selected:
            return redirect("reorder-alerts")
        return self._render(request, {k: Decimal(str(v)) for k, v in selected.items()})
    ...
    def post(self, request):
        ...
        if not selected:
            messages.warning(
                request,
                "Restock Quantity must be above 0 to calculate components. Adjust the quantities and try again.",
            )
            return redirect("reorder-alerts")
        ...
        for mat_id, required_qty in sorted(
            component_totals.items(), key=lambda x: component_materials[x[0]].sku
        ):
            ...
            raw_stock = RawMaterialBatch.objects.filter(material=mat).aggregate(
                t=DSum("total_quantity")
            )["t"] or Decimal("0")
            allocated = RawBatchAllocation.objects.filter(
                raw_batch__material=mat
    ...
    def _render(self, request, selected):
        ...
        for mat_id, required_qty in sorted(
            ...
            gap = in_stock - required_qty
            pallets = (
                mat.pallets_for_qty(float(required_qty))
        ...
        if sort in ("sku", "name", "in_stock", "required_qty", "gap"):
            rows = sorted(
                rows,
                key=lambda r: (
                    r[sort] if sort not in ("sku", "name") else r[sort].lower()
                ),
                reverse=reverse,
            )
...
class ReorderComponentsExportView(View):
    def get(self, request):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="components-{date.today()}.xlsx"'
        )
        wb.save(response)
        return response
...
_STOCK_REPORT_ROWS = [
    {
        "name": "Christmas Santa 500 ml",
        "name_color": "FFF4CCCC",
        "breakdown": "Christmas Santa 500 ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000386",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Christmas Santa 500 ml US - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000387",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Christmas Santa 500 ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000086",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Christmas Santa 500 ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000069",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Christmas Santa 500 ml - US label filled",
        "is_raw": False,
        "sku": "02-0000070",
    },
    {
        "name": "Snowman 500 ml",
        "name_color": "FFC9DAF8",
        "breakdown": "Snowman 500 ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000384",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Snowman 500 ml US - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000385",
    },
    {
    ...
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Snowman 500 ml - US label filled",
        "is_raw": False,
        "sku": "02-0000097",
    },
    {
        "name": "Black 500ml",
        "name_color": "FF434343",
        "breakdown": "Black 500ml KR - Empty painted bottles (Pepper)",
        "is_raw": True,
        "sku": "07-0000380",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Black 500ml KR - Empty painted bottles (Berlin)",
        "is_raw": True,
        "sku": "07-0000136",
    },
    {
    ...
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Black 500ml US - Empty painted bottles (Berlin)",
        "is_raw": True,
        "sku": "07-0000137",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Black 500ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000055",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Black 500ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000027",
    },
    {
    ...
    },
    {
        ...
        "name_color": "FFEAD1DC",
        "breakdown": "Valentine 500ml KR - Empty painted bottles (Berlin)",
        "is_raw": True,
        "sku": "07-0000139",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Valentine 500ml EU - Empty painted bottles (Pepper)",
        "is_raw": True,
        "sku": "07-0000434",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Valentine 500ml EU - Empty painted bottles (Berlin)",
        "is_raw": True,
        "sku": "07-0000138",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Valentine 500ml US - Empty painted bottles (Pepper)",
        "is_raw": True,
        "sku": "07-0000435",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Valentine 500ml US - Empty painted bottles (Berlin)",
        "is_raw": True,
        "sku": "07-0000140",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Valentine 500ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000007",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Valentine 500ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000028",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Valentine 500ml - US label filled",
        "is_raw": False,
        "sku": "02-0000016",
    },
    {
        "name": "Extra virgin olive oil with basil, bottle 200ml",
        "name_color": "ffbbdab0",
        "breakdown": "Extra virgin olive oil with basil, bottle 200ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000128",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with basil, bottle 200ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000001",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with basil, bottle 200ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000020",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with basil, bottle 200ml - US label filled",
        "is_raw": False,
        "sku": "02-0000010",
    },
    {
        "name": "Extra virgin olive oil with lemon, bottle 200ml",
        "name_color": "ffffe599",
        "breakdown": "Extra virgin olive oil with lemon, bottle 200ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000127",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with lemon, bottle 200ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000002",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with lemon, bottle 200ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000011",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with lemon, bottle 200ml - US label filled",
        "is_raw": False,
        "sku": "02-0000021",
    },
    {
        "name": "Extra virgin olive oil with chili, bottle 200ml",
        "name_color": "fff3946a",
        "breakdown": "Extra virgin olive oil with chili, bottle 200ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000125",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with chili, bottle 200ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000004",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with chili, bottle 200ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000022",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with chili, bottle 200ml - US label filled",
        "is_raw": False,
        "sku": "02-0000013",
    },
    {
        "name": "Extra virgin olive oil with garlic, bottle 200ml",
        "name_color": "ffC29F80",
        "breakdown": "Extra virgin olive oil with garlic, bottle 200ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000126",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with garlic, bottle 200ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000005",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with garlic, bottle 200ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000023",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with garlic, bottle 200ml - US label filled",
        "is_raw": False,
        "sku": "02-0000014",
    },
    {
        "name": "Extra virgin olive oil with truffle, bottle 200ml",
        "name_color": "ff674436",
        "breakdown": "Extra virgin olive oil with truffle, bottle 200ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000129",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with truffle, bottle 200ml - KR label filled",
        "is_raw": False,
        "sku": "02-0000003",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with truffle, bottle 200ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000024",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with truffle, bottle 200ml - US label filled",
        "is_raw": False,
        "sku": "02-0000012",
    },
    {
        "name": "Extra virgin olive oil with tomato, bottle 200ml",
        "name_color": "FFF4CCCC",
        "breakdown": "Extra virgin olive oil with tomato, bottle 200ml KR - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000309",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with tomato, bottle 200ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000124",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with tomato, bottle 200ml - US label filled",
        "is_raw": False,
        "sku": "02-0000095",
    },
    {
        "name": "Aged Vinegar 200ml",
        "name_color": "FFD9D2E9",
        "breakdown": "Aged Vinegar 200ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000156",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Aged Vinegar 200ml US - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000330",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Aged Vinegar 200ml - KR label filled",
        "is_raw": False,
        "sku": "04-0000015",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Aged Vinegar 200ml - EU label filled",
        "is_raw": False,
        "sku": "04-0000012",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Aged Vinegar 200ml - US label filled",
        "is_raw": False,
        "sku": "04-0000016",
    },
    {
        "name": "White Vinegar 200ml",
        "name_color": "FFF9CB9C",
        "breakdown": "White Vinegar 200ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000285",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "White Vinegar 200ml US - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000331",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "White Vinegar 200ml - KR label filled",
        "is_raw": False,
        "sku": "04-0000014",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "White Vinegar 200ml - EU label filled",
        "is_raw": False,
        "sku": "04-0000013",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "White Vinegar 200ml - US label filled",
        "is_raw": False,
        "sku": "04-0000017",
    },
    {
        "name": "Salt, 150gr, jar",
        "name_color": "FF9FC5E8",
        "breakdown": "Salt, 150gr, jar - yiayia salt blue 212ml Berlin",
        "is_raw": True,
        "sku": "07-0000355",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Salt, 150gr, jar - EU label filled",
        "is_raw": False,
        "sku": "06-0000008",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Salt, 150gr, jar - US label filled",
        "is_raw": False,
        "sku": "06-0000009",
    },
        {
        "name": "Extra virgin olive oil with basil, bottle 50ml",
        "name_color": "ffbbdab0",
        "breakdown": "Extra virgin olive oil with basil, bottle 50ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000133",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with basil, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000044",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with basil, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000032",
    },
    {
        "name": "Extra virgin olive oil with lemon, bottle 50ml",
        "name_color": "ffffe599",
        "breakdown": "Extra virgin olive oil with lemon, bottle 50ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000132",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with lemon, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000047",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with lemon, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000037",
    },
    {
        "name": "Extra virgin olive oil with chili, bottle 50ml",
        "name_color": "fff3946a",
        "breakdown": "Extra virgin olive oil with chili, bottle 50ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000130",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with chili, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000043",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with chili, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000035",
    },
    {
        "name": "Extra virgin olive oil with garlic, bottle 50ml",
        "name_color": "ffC29F80",
        "breakdown": "Extra virgin olive oil with garlic, bottle 50ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000131",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with garlic, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000045",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with garlic, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000034",
    },
    {
        "name": "Extra virgin olive oil with truffle, bottle 50ml",
        "name_color": "ff674436",
        "breakdown": "Extra virgin olive oil with truffle, bottle 50ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000134",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with truffle, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000046",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with truffle, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000036",
    },
    {
        "name": "Extra virgin olive oil with tomato, bottle 50ml",
        "name_color": "FFF4CCCC",
        "breakdown": "Extra virgin olive oil with tomato, bottle 50ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000316",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil with tomato, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000125",
    },
    {
        "name": "Extra virgin olive oil, bottle 50ml",
        "name_color": "FF4c4c4c",
        "breakdown": "Extra virgin olive oil, bottle 50ml EU - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000182",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000025",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000033",
    },
    {
        "name": "Extra virgin olive oil tree, bottle 50ml",
        "name_color": "FF47865F",
        "breakdown": "Extra virgin olive oil tree, bottle 50ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000289",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil tree, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000079",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil tree, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000106",
    },
    {
        "name": "Extra virgin olive oil star, bottle 50ml",
        "name_color": "FFFFD453",
        "breakdown": "Extra virgin olive oil star, bottle 50ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000291",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil star, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000080",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil star, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000107",
    },
    {
        "name": "Extra virgin olive oil snowman, bottle 50ml",
        "name_color": "FF0077DB",
        "breakdown": "Extra virgin olive oil snowman, bottle 50ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000293",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil snowman, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000081",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil snowman, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000108",
    },
    {
        "name": "Extra virgin olive oil reindeer, bottle 50ml",
        "name_color": "FFF2EFEE",
        "breakdown": "Extra virgin olive oil reindeer, bottle 50ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000292",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil reindeer, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000082",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil reindeer, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000109",
    },
    {
        "name": "Extra virgin olive oil bell, bottle 50ml",
        "name_color": "FFE6609D",
        "breakdown": "Extra virgin olive oil bell, bottle 50ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000294",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil bell, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000083",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil bell, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000110",
    },
    {
        "name": "Extra virgin olive oil santa, bottle 50ml",
        "name_color": "FFD10000",
        "breakdown": "Extra virgin olive oil santa, bottle 50ml - Empty painted bottles",
        "is_raw": True,
        "sku": "07-0000290",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil santa, bottle 50ml - EU label filled",
        "is_raw": False,
        "sku": "02-0000085",
    },
    {
        "name": None,
        "name_color": None,
        "breakdown": "Extra virgin olive oil santa, bottle 50ml - US label filled",
        "is_raw": False,
        "sku": "02-0000111",
    },
]
...
class StockReportView(View):
    def get(self, request):
        from django.db.models import Sum as DSum
        from .models import RawMaterialBatch, ProductBatch

        raw_stock = {
            r["material__sku"]: r["total"]
            for r in RawMaterialBatch.objects.values("material__sku").annotate(
                total=DSum("total_quantity")
            )
        }
        fin_stock = {
            r["material__sku"]: r["total"]
            for r in ProductBatch.objects.values("material__sku").annotate(
                total=DSum("quantity_produced")
            )
        }
        ...
        last_sync = None
        last_raw = (
            RawMaterialBatch.objects.filter(lot_number__startswith="ERP-SYNC-")
            .order_by("-created_at")
            .first()
        )
        if last_raw:
        ...
        return render(
            request, "stock_report/report.html", {"rows": rows, "last_sync": last_sync}
        )
